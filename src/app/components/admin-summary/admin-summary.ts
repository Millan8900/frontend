...
export default class AdminSummary {
  ...
  loadUsers() {
    this.http.get<any[]>('https://backend-production-35363.up.railway.app/api/admin/users').subscribe({
      next: (data) => {
        this.users = data;
  ...
  assignRole(id: number, role: string) {
    this.http.put(
      `https://backend-production-35363.up.railway.app/api/admin/users/${id}/role?newRole=${role}`,
      {},
      { responseType: 'text' }
    ).subscribe({
      next: (msg) => {
  ...
  deleteUser(id: number, email: string) {
    ...
    if (confirm(`Are you sure you want to delete ${email}?`)) {
      this.http.delete(`https://backend-production-35363.up.railway.app/api/admin/users/${id}`, { responseType: 'text' }).subscribe({
        next: (msg) => {
          alert(msg);
          this.loadUsers();
        },
