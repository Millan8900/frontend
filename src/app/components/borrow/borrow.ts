...
export default class Borrow implements OnInit {
 ...
 loadBorrowedBooks() {
  ...
  this.http.get<any[]>(`https://backend-production-35363.up.railway.app/api/borrow/user/email/${encodedEmail}`)
    .subscribe({
  ...
  returnBook(recordId: number) {
    ...
    if (confirm('Are you sure you want to return this book?')) {
      this.http.post(`https://backend-production-35363.up.railway.app/api/borrow/return/${recordId}`, {}, { responseType: 'text' })
        .subscribe({
          next: (response) => {
