...
export default class Student implements OnInit {
  ...
  loadBooks(): void {
    this.loading = true;
    this.http.get<any[]>('https://backend-production-35363.up.railway.app/api/books').subscribe({
      next: (data: any) => {
  ...
  requestBorrow(bookId: number): void {
    ...
    this.http.post('https://backend-production-35363.up.railway.app/api/borrow/request', borrowRequest).subscribe({
      next: (response: any) => {
